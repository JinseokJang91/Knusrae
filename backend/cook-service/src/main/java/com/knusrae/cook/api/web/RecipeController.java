...
//            @Valid @RequestPart(value = "recipe") RecipeDto recipeDto,
        ...
        log.info("Creating recipe: {}, images: {}, mainImageIndex: {}", recipeJson, images, mainImageIndex);
        RecipeDto dto = mapper.readValue(recipeJson, RecipeDto.class);
        log.info("Parsed recipeDto: {}", dto.toString());
        RecipeDto created = recipeService.createRecipe(dto, images, mainImageIndex);
        log.info("Created recipe: {}", created.toString());
...
//        return ResponseEntity.status(HttpStatus.CREATED).body(createdRecipe);
    ...
    // 비활성화: 논리삭제 API는 현재 미사용

    // 비활성화: 검색/통계/인기/페이징/사용자별 등 확장 API는 현재 미사용

    // 비활성화: 레거시 save 엔드포인트는 현재 미사용
}
