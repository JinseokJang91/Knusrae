...
export async function getAdminCommonCodeList(): Promise<CommonCodeListItem[]> {
    const response = await httpJson<CommonCodeListItem[] | unknown>(
        BASE_URL,
...
export async function getAdminCommonCodeOne(codeId: string): Promise<AdminCommonCode> {
    return httpJson<AdminCommonCode>(
        BASE_URL,
        `/api/admin/common-codes/${encodeURIComponent(codeId)}`,
        { method: 'GET' }
    );
}
...
/** 관리자: 상세코드 추가 */
export async function createCommonCodeDetail(
    codeId: string,
    request: CommonCodeDetailCreateRequest
): Promise<AdminCommonCodeDetailItem> {
    return httpJson<AdminCommonCodeDetailItem>(
        BASE_URL,
        `/api/admin/common-codes/${encodeURIComponent(codeId)}/details`,
        {
            method: 'POST',
            body: JSON.stringify(request)
        }
...
/** 관리자: 상세코드 수정 */
export async function updateCommonCodeDetail(
    codeId: string,
    detailCodeId: string,
    request: CommonCodeDetailUpdateRequest
): Promise<AdminCommonCodeDetailItem> {
    return httpJson<AdminCommonCodeDetailItem>(
        BASE_URL,
        `/api/admin/common-codes/${encodeURIComponent(codeId)}/details/${encodeURIComponent(detailCodeId)}`,
        {
            method: 'PUT',
            body: JSON.stringify(request)
        }
