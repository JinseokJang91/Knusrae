...
<template>
    <div v-if="ingredientGroups && ingredientGroups.length > 0" class="bg-white rounded-2xl shadow-lg p-8 mb-8">
        ...
        <div class="space-y-6">
            <div v-for="(group, groupIndex) in ingredientGroups" :key="`group-${groupIndex}-${group.order}`" class="bg-orange-50 rounded-xl p-6">
                ...
                <div v-if="group.items && group.items.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div v-for="(item, itemIndex) in group.items" :key="`item-${groupIndex}-${itemIndex}-${item.name}`" class="flex items-center p-3 bg-white rounded-lg border border-orange-200">
