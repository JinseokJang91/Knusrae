        ...
        <div class="flex flex-col gap-6">
                ...
                <div v-else>
                    ...
                    <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div
                            ...
                            :data-category-id="`category-${option.codeId}`"
                        >
            ...
            <div>
                ...
                <div v-else>
                    ...
                    <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div
                            v-for="option in cookingTipsOptions"
                            :key="option.codeId"
                            class="flex flex-col gap-2"
                            :data-cooking-tip-id="`cookingTip-${option.codeId}`"
                        >
                            <span class="text-sm font-medium text-gray-700">{{ option.codeName }}</span>
                            <Select
...
function getDetailOptions(option: CommonCodeOption): Array<{ label: string; value: string }> {
    return [
        { label: '선택하세요', value: '' },
        ...(option.details || []).map((d) => ({ label: d.codeName, value: d.detailCodeId }))
    ];
}
</script>
