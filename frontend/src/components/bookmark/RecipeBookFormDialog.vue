...
const saveRecipeBook = async () => {
    ...
    try {
        ...
        console.error('레시피북 저장 실패:', e);
        const message = e instanceof Error ? e.message : String(e);
        if (message.includes('409')) {
            showError('이미 존재하는 레시피북 이름입니다.');
        } else {
            showError(isEditMode.value ? '레시피북 수정에 실패했습니다.' : '레시피북 생성에 실패했습니다.');
        }
    } finally {
        loading.value = false;
    }
...
<template>
    <Dialog v-model:visible="visible" modal :header="isEditMode ? '레시피북 수정' : '새 레시피북 만들기'" :style="{ width: '450px' }" :draggable="false">
        <div class="recipe-book-form">
            <!-- 레시피북 이름 -->
            <div class="mb-4">
                <label class="block mb-2 font-medium">레시피북 이름</label>
                <InputText v-model="recipeBookName" placeholder="예: 한식 레시피, 간식 모음" class="w-full" :maxlength="50" @keyup.enter="saveRecipeBook" />
            ...
            <div class="mb-4">
                ...
                <div class="color-grid">
                    <div v-for="color in RECIPE_BOOK_COLORS" :key="color.name" class="color-option" :class="{ 'color-option-selected': selectedColor === color.name }" @click="selectedColor = color.name">
                        <div class="color-circle" :style="{ backgroundColor: color.hex }"></div>
                        <span class="color-label">{{ color.label }}</span>
                        <i v-if="selectedColor === color.name" class="pi pi-check text-primary"></i>
                    </div>
                </div>
            </div>
            ...
            <div class="preview-box">
                ...
                <div class="preview-content">
                    <i class="pi pi-bookmark text-3xl" :style="{ color: getRecipeBookColorHex(selectedColor) }"></i>
                    <div class="ml-3">
