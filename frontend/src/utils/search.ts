import { searchRecipes as searchRecipesApi } from '@/api/searchApi';
import type { Recipe } from '@/types/recipe';

/**
 * 검색 서비스
 * 레시피 검색 API 호출 및 최근 검색어(localStorage) 관리
 */
...
export async function searchRecipes(keyword: string): Promise<Recipe[]> {
    try {
        return await searchRecipesApi(keyword);
    } catch (error) {
