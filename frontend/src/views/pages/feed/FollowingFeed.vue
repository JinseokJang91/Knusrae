...
<template>
    <div class="page-container page-container--card">
        <div class="following-feed-container">
            ...
            <!-- 로그인하지 않은 경우 -->
            <PageStateBlock v-if="!isLoggedIn" state="empty" empty-icon="pi pi-lock" empty-title="로그인이 필요합니다" empty-message="팔로잉 피드를 보려면 로그인해 주세요." empty-button-label="로그인하기" @empty-action="goToLogin" />

            <!-- 로딩 상태 -->
            <PageStateBlock v-else-if="loading" state="loading" loading-message="레시피를 불러오는 중..." />

            <!-- 에러 상태 -->
            <PageStateBlock v-else-if="error" state="error" error-title="레시피를 불러올 수 없습니다" :error-message="error" retry-label="다시 시도" @retry="loadFeed" />
            ...
            <div v-else class="feed-content">
                <div class="recipe-grid">
                    <RecipeCard v-for="recipe in recipes" :key="recipe.id" :recipe="recipe" @click="goToRecipeDetail(recipe.id)" />
                </div>

                <!-- 페이지네이션 -->
                <div v-if="totalPages > 1" class="flex justify-center mt-6">
                    <Paginator v-model:first="first" :rows="pageSize" :totalRecords="totalRecipes" @page="onPageChange" />
                </div>

                <!-- 더 불러오기 버튼 (무한 스크롤 대신) -->
                <div v-if="hasMore" class="flex justify-center mt-4">
                    <Button label="더 보기" icon="pi pi-refresh" outlined @click="loadMore" :loading="loadingMore" />
                </div>
            </div>
        </div>
</template>
